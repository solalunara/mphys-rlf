...
def load_optical_catalogue(file_path="optical_catalogue/combined-release-v1.2-LM_opt_mass.fits"):
    """
...
def get_optical_positions(optical_catalogue):
    """
...
if __name__ == "__main__":
    logger = utils.logging.get_logger("downloading optical files for database creation", logging.DEBUG)

    # Prepare for distributed processing on galahad
    du = DistributedUtils()
    task_count = du.get_task_count()
    task_id = du.get_task_id()
    logger.debug(f'Task ID: {task_id}, Task Count: {task_count}')
    ...
    opt_catalogue = load_optical_catalogue()
    opt_positions = get_optical_positions(opt_catalogue)
    ...
    if isinstance(n_cpus, str):
        n_cpus = int(n_cpus)
    logger.info("Using %i cpu" + ("s" if n_cpus != 1 else ""), n_cpus)
    ...
    for i in tqdm(image_nums):
        ...
        get_cutout(f"optical_catalogue/dr2_cutouts_download/cutout{i}.fits", f"{ra} {dec}")
